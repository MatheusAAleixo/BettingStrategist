...
def calcular_apostas(valor_total, odd_aposta1, odd_aposta2, odd_aposta3):
    ...
    odd_aposta3 = Decimal(str(odd_aposta3))  # Converta para Decimal
...
def processar_excel(entrada_excel, saida_excel):
    ...
    for index, row in df.iterrows():
        resultado = calcular_apostas(row.iloc[0], row.iloc[1], row.iloc[2], row.iloc[3])
    ...
    df['aposta3'] = [f"Aposta 3: Apostei {res[2]:.2f}" if res else None for res in resultados]
...
caminho_arquivo_odds = r'C:\Users\Matheus A Aleixo\Documents\WORKSPACE_PYTHON\script_aposta_excel\ODDS3.xlsx'
